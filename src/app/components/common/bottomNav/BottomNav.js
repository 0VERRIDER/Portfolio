        ...
        <div className="flex flex-row w-full h-20 backdrop-blur-sm z-50 fixed bottom-0 left-0 justify-between items-center px-5 select-none">
            <div className="flex flex-row border-white border rounded-full items-center justify-evenly w-auto px-4 h-9"
                 onClick={
                    () => {
                        showBottomMessageBar(
                            "Its an under development chat feature.", 
                            "bg-yellow-700"
                        )
                    }
                }>
                <span className='relative flex h-3 w-3 mr-3'>
                    ...
                    <div className="status-text text-sm text-white font-normal lowercase">
                        online
                    </div>
            </div>
            <button onClick={toggleMenu}>
                {
                    isMenuOpen ? 
                    <XMarkIcon className="text-white h-9 w-9 border border-white p-1 rounded" /> : 
